...
export class BotService {
  ...
  private filePathPattern = /chunk_(\d+)\.webm/;
  constructor(
    ...
    if (!fs.existsSync(this.folderPath)) {
      this.logger.log(`Folder path does not exist, creating folder ${this.folderPath}`);
      fs.mkdirSync(this.folderPath);
      this.logger.log(`Folder created!`);
    } else {
      this.logger.log(`Folder path exists, deleting files in folder ${this.folderPath}`);
    ...
    this.watcher.on('add', async (filePath) => {
      ...
      try {
        ...
        fs.unlink(filePath, (err) => {
          if (err) {
            throw new InternalServerErrorException(`Error deleting file ${filePath}: ${err}`);
          } else {
        ...
        throw new InternalServerErrorException(`Error in startWatcher function: ${error}`);
      }
    });
  }
  ...
  private async joinGoogleMeet(page: any, meetingUrl: string) {
    try {
      ...
      throw new InternalServerErrorException(`Error in joinGoogleMeet function: ${error}`);
    }
  }
      ...
      setTimeout(
        async () => {
          try {
            ...
            throw new InternalServerErrorException(`Error in record function: ${error}`);
          }
        },
